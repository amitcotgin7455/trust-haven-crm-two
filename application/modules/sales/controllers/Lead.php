...
class Lead extends CI_Controller {
    ...
    public function update(){
        ...
        $data['exist_lead_detail'] = $lead_datail;
        $getupdate_id = $this->LeadNoteModel->getData($idG);
        if(!empty($getupdate_id)){
            ...
            $data['active'] = 'active';
            $data['editResult'] = $this->LeadNoteModel->getData($idG);
            $data['getemail'] = $this->LeadNoteModel->getListsendemail($prei);
            $data['getedraft'] = $this->LeadNoteModel->getListdraft($prei);
            $data['template'] = $this->LeadNoteModel->getTemplate();
            $this->load->view('include/header', $data);
            $this->load->view('lead/index',$data);
            $this->load->view('include/footer',$data);
        }
    ...
    public function addLead(){   
        ...
        if((!isset($_POST['save'])))
        {
            redirect(base_url('sales/create-lead'));
            exit();
        } 
        if(isset($_POST['save'])){
       ...
       if($this->getLeadUnqiue($email,$phone))
        {
                ...
                redirect(base_url() . 'sales/create-lead?error='.base64_encode(json_encode($insert)));
        }
        else
        {
      

        $res = $this->LeadNoteModel->insertData('tbl_leads', $insert);
        // log activity 
        $res = $this->db->insert_id();
        $user_id           = $data['user_detail']->id;
        $id           = $res;
        $log_title         = "Add Lead Sales Crm - " . json_encode($insert);
        $log_create        = app_log_manage($user_id, $id, $log_title);
        // log activity 
        if ($res) {
            $_SESSION['done'] = 'Lead Data Added successfully';
            redirect(base_url().'sales/list-lead');
        } else {
...
public function updateLead(){   
    ...
    $update = array(
     ...
     {
             $_SESSION['error'] = "This Lead already exist";
             redirect(base_url() . 'sales/update-lead/?id='.base64_encode($idH).'&error='.base64_encode(json_encode($update)));
     }
     else
     {
     
     if($lead_status==2){
        $countlead = $this->LeadNoteModel->countLeadTransfer($idH);
        if($countlead){
            $data = array(
                'first_name' => $name,
                'last_name' => $last_name,
            ...
            $insert = $this->LeadNoteModel->insertTransferLead($data);
    ...
    $user_id           = $data['user_detail']->id;
    $id           = $insert_id;
    $log_title         = "Update Lead Sales Crm  - " . json_encode($update);
    $log_create        = app_log_manage($user_id, $id, $log_title);
    // log activity 
     if ($this->LeadNoteModel->updateRecord('lead_id', $idH, 'tbl_leads',$update)) {
...
public function listLead()
	{	     
        ...
        if ($data['user_detail']->role_id == 1 || $data['user_detail']->role_id == 4) {
            if(!empty($getuser_data)){
                $data['result'] = $this->LeadNoteModel->SearchUserData($getuser_data);
            }
            elseif(!empty($gsearch)){
                $data['result'] = $this->LeadNoteModel->getSearchInput_admin($gsearch);
            }
            elseif(!empty($date_data_sale)){
                $data['result'] = $this->LeadNoteModel->getDateofSale($date_data_sale);
            }
            elseif(!empty($lead_status)){
                $data['result'] = $this->LeadNoteModel->getLeadStatus($lead_status);
            }
            elseif($getdays=='days'){
                $getnumber1=$getnumber;
                $data['result'] = $this->LeadNoteModel->getdatedays($getnumber1);
            }
            elseif($getdays=='Weeks'){
                $getnumber1=$getnumber;
                $data['result'] = $this->LeadNoteModel->getdateweek($getnumber1);
            }
            elseif($getdays=='Months'){
                $getnumber1=$getnumber;
                $data['result'] = $this->LeadNoteModel->getdatemonths($getnumber1);
            }
            elseif($time_range=='Today'){
                $data['result'] = $this->LeadNoteModel->getdatetoday();
            }
            elseif($time_range=='This week'){
                $data['result'] = $this->LeadNoteModel->getdateThisweek();
            }
            elseif($time_range=='This month'){
                $data['result'] = $this->LeadNoteModel->getdateThismonth();
            }
            elseif($time_range=='This year'){
                $data['result'] = $this->LeadNoteModel->getdateThisyear();
            }
            elseif($time_range=='in the last 30 days'){
                $data['result'] = $this->LeadNoteModel->getLast30days();
            }

            elseif($time_range=='in the last 60 days'){
                $data['result'] = $this->LeadNoteModel->getLast60days();
            }

            elseif($time_range=='in the last 90 days'){
                $data['result'] = $this->LeadNoteModel->getLast90days();
            }

            elseif(!empty($from and $too)){
                $from1=$from;
                $too1=$too;
                $data['result'] = $this->LeadNoteModel->getFromToo($from1,$too1);
            }
            else{  
            $data['result'] = $this->LeadNoteModel->getResultAdmin($tableName,$DbKey,$idG,$order_id);
            }
            $data['searchuser'] = $this->LeadNoteModel->Searchuser();
         }
         else{
            if(!empty($gsearch)){
                $data['result'] = $this->LeadNoteModel->getSearchInput_admin_user($gsearch,$id);
            }
            elseif(!empty($date_data_sale)){
                $data['result'] = $this->LeadNoteModel->getDateofSale_user($date_data_sale,$id);
            }
            elseif(!empty($lead_status)){
                $data['result'] = $this->LeadNoteModel->getLeadStatus_user($lead_status,$id);
            }
            elseif($getdays=='days'){
                $getnumber1=$getnumber;
                $data['result'] = $this->LeadNoteModel->getdatedays_user($getnumber1,$id);
            }
            elseif($getdays=='Weeks'){
                $getnumber1=$getnumber;
                $data['result'] = $this->LeadNoteModel->getdateweek_user($getnumber1,$id);
            }
            elseif($getdays=='Months'){
                $getnumber1=$getnumber;
                $data['result'] = $this->LeadNoteModel->getdatemonths_user($getnumber1,$id);
            }
            elseif($time_range=='Today'){
                $data['result'] = $this->LeadNoteModel->getdatetoday_user($id);
            }
            elseif($time_range=='This week'){
                $data['result'] = $this->LeadNoteModel->getdateThisweek_user($id);
            }
            elseif($time_range=='This month'){
                $data['result'] = $this->LeadNoteModel->getdateThismonth_user($id);
            }
            elseif($time_range=='This year'){
                $data['result'] = $this->LeadNoteModel->getdateThisyear_user($id);
            }
            elseif($time_range=='in the last 30 days'){
                $data['result'] = $this->LeadNoteModel->getLast30days_user($id);
            }

            elseif($time_range=='in the last 60 days'){
                $data['result'] = $this->LeadNoteModel->getLast60days_user($id);
            }

            elseif($time_range=='in the last 90 days'){
                $data['result'] = $this->LeadNoteModel->getLast90days_user($id);
            }

            elseif(!empty($from and $too)){
                $from1=$from;
                $too1=$too;
                $data['result'] = $this->LeadNoteModel->getFromToo_user($from1,$id,$too1);
            }
            else{

                $data['result'] = $this->LeadNoteModel->getResultUser($id);
            }
    ...
    public function save() {
        ...
        $data = $this->LeadNoteModel->saveEmp();
        // log activity 
        $insert_id = $this->db->insert_id();
        $user_id           = $userid;
        $id           = $insert_id;
        $log_title         = "Add Notes Lead Module Sales Crm  - " . $this->input->post('note');
        $log_create        = app_log_manage($user_id, $id, $log_title);
        // log activity
    ...
    function show(){
        $idG = $this->input->get('id');
        $data=$this->LeadNoteModel->employeeList($idG);
		echo json_encode($data);
    ...
    function delete(){
        $data['user_detail'] = $this->auth->user_info();
        $userid = $data['user_detail']->id; 
        $data=$this->LeadNoteModel->deleteEmp();
         // log activity 
         $insert_id = $this->input->post('id');
         $user_id           = $userid;
         $id           = $insert_id;
         $log_title         = "Delete Notes id Lead Module Sales Crm - " . $id;
         $log_create        = app_log_manage($user_id, $id, $log_title);
         // log activity 
		echo json_encode($data);
	}
    function updatenote(){
        $data['user_detail'] = $this->auth->user_info();
        $userid = $data['user_detail']->id; 
        $data=$this->LeadNoteModel->updateEmp();
        // log activity 
        $insert_id = $this->input->post('id');
        $user_id           = $userid;
        $id           = $insert_id;
        $log_title         = "Update Notes Lead Module  Sales Crm - " . $this->input->post('note');
        $log_create        = app_log_manage($user_id, $id, $log_title);
        // log activity 
		echo json_encode($data);
	}
    ...
    {   
        ...
        $user_id           = $userid;
        $id           = $insert_id;
        $log_title         = "Delete Email Lead Module Sales Crm - " . $ids;
        $log_create        = app_log_manage($user_id, $id, $log_title);
        // log activity 
    ...
    {   
        ...
        $user_id           = $userid;
        $id           = $insert_id;
        $log_title         = "Delete Email Drafts Lead Module Sales Crm - " . $ids;
        $log_create        = app_log_manage($user_id, $id, $log_title);
        // log activity
    ...
    {   $data['user_detail'] = $this->auth->user_info();
        $ids = $this->input->post('ids');
        $delete = $this->LeadNoteModel->deleteLead($ids);  
        // log activity 
        $insert_id = $ids;
        $user_id           = $data['user_detail']->id;
        $id           = $insert_id;
        $log_title         = "Delete Lead id Sales Crm - " . $ids;
        $log_create        = app_log_manage($user_id, $id, $log_title);
        // log activity 
        if($delete){
    ...
    public function emaildraft(){
        ...
        $insert = $this->LeadNoteModel->insertDraft($savedraft);
        // log activity 
        $insert_id = $this->db->insert_id();
        $user_id           = $data['user_detail']->id;
        $id           = $insert_id;
        $log_title         = "Add Email Draft Lead Module Sales Crm  - " . json_encode($savedraft);
        $log_create        = app_log_manage($user_id, $id, $log_title);
        // log activity
        if($insert){
            echo 1;
   ...
   public function mail(){
    ...
    $save = $this->LeadNoteModel->sendEmail($insert);
    // log activity 
    $insert_id = $this->db->insert_id();
    $user_id           = $userid;
    $id           = $insert_id;
    $log_title         = "Add Email And Send Email Lead Module Sales Crm  - " . json_encode($insert);
    $log_create        = app_log_manage($user_id, $id, $log_title);
    // log activity
...
public function fetchtemplate(){
    ...
    $data = $this->LeadNoteModel->fetchTemplate($id);
   echo  json_encode($data);
...
public function mailForm(){
    $data['activeSender'] = active_mail();
    $this->load->view('massemail_modal_form/index',$data);
}
