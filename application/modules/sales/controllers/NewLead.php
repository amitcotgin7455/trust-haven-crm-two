...
class NewLead extends CI_Controller {
	...
	{	
        ...
        $data['template'] = $this->NewLeadNoteModel->getTemplate();
		...
		$this->load->view('newlead/index',$data);
    ...
    public function update(){
        ...
        $data['exist_lead_detail'] = $lead_datail;
        $getupdate_id = $this->NewLeadNoteModel->getData($idG);
        if(!empty($getupdate_id)){
            ...
            $data['name'] = $data['user_detail']->name;
            $data['user_role'] = $data['user_detail']->role_type;
            $data['title'] = 'Update New Lead';
            $data['active1'] = 'active';
            $data['editResult'] = $this->NewLeadNoteModel->getData($idG);
            $data['getemail'] = $this->NewLeadNoteModel->getListsendemail($prei);
            $data['getedraft'] = $this->NewLeadNoteModel->getListdraft($prei);
            $data['template'] = $this->NewLeadNoteModel->getTemplate();
            $this->load->view('include/header', $data);
            $this->load->view('newlead/index',$data);
            $this->load->view('include/footer',$data);
    ...
    public function addNewLead(){  
        ...
        if(isset($_POST['save'])){
           ...
           if($this->getLeadUnqiue($email,$phone))
            ...
            redirect(base_url() . 'sales/create-newlead?error='.base64_encode(json_encode($insert)));
            }
            else
            {

    
            $res = $this->NewLeadNoteModel->insertData('tbl_leads', $insert);
            // log activity 
            $res = $this->db->insert_id();
            $user_id           = $data['user_detail']->id;
            $id           = $res;
            $log_title         = "Add New Lead Sales Crm - " . json_encode($insert);
            $log_create        = app_log_manage($user_id, $id, $log_title);
            // log activity
            if ($res) {
                $_SESSION['done'] =  'Lead Data Added successfully';
                redirect(base_url().'sales/list-newlead');
            } else {
                $_SESSION['done'] =  'Lead Data Not Added successfully';
                redirect(base_url().'sales/list-newlead');
           }
         }
        }
...
public function updateNewLead(){   
    ...
    $update = array(
     ...
     {
             $_SESSION['error'] = "This Lead already exist";
             redirect(base_url() . 'sales/update-newlead?id='.base64_encode($idH).'&error='.base64_encode(json_encode($update)));
     }
     else
     {  
     ...
     if($lead_status==2){
        ...
        $user_id           = $data['user_detail']->id;
        $id           = $insert_id;
        $log_title         = "Update New Lead Sales Crm  - " . json_encode($update);
        $log_create        = app_log_manage($user_id, $id, $log_title);
        // log activity 
     if ($this->NewLeadNoteModel->updateRecord('lead_id', $idH, 'tbl_leads',$update)) {
         $_SESSION['done'] = 'Lead Data Successfully updated...!!';
         redirect(base_url().'sales/list-newlead');
     } else {
         $_SESSION['error'] =  'Lead Data is not successfully updated...!!';
         redirect(base_url().'sales/list-newlead');
        }
  }
...
public function NewlistLead()
	{	     
        ...
        $data['user_role'] = $data['user_detail']->role_type;
		$data['active1'] = 'active';
		$data['title'] = 'New Lead List';

        if ($data['user_detail']->role_id == 1 || $data['user_detail']->role_id == 4) {
            if(!empty($getuser_data)){
                $data['result'] = $this->NewLeadNoteModel->SearchUserData($getuser_data);
            }
            elseif(!empty($gsearch)){
                $data['result'] = $this->NewLeadNoteModel->getSearchInput_admin($gsearch);
            }
            elseif(!empty($date_data_sale)){
                $data['result'] = $this->NewLeadNoteModel->getDateofSale($date_data_sale);
            }
            elseif(!empty($lead_status)){
                $data['result'] = $this->NewLeadNoteModel->getLeadStatus($lead_status);
            }
            elseif($getdays=='days'){
                $getnumber1=$getnumber;
                $data['result'] = $this->NewLeadNoteModel->getdatedays($getnumber1);
            }
            elseif($getdays=='Weeks'){
                $getnumber1=$getnumber;
                $data['result'] = $this->NewLeadNoteModel->getdateweek($getnumber1);
            }
            elseif($getdays=='Months'){
                $getnumber1=$getnumber;
                $data['result'] = $this->NewLeadNoteModel->getdatemonths($getnumber1);
            }
            elseif($time_range=='Today'){
                $data['result'] = $this->NewLeadNoteModel->getdatetoday();
            }
            elseif($time_range=='This week'){
                $data['result'] = $this->NewLeadNoteModel->getdateThisweek();
            }
            elseif($time_range=='This month'){
                $data['result'] = $this->NewLeadNoteModel->getdateThismonth();
            }
            elseif($time_range=='This year'){
                $data['result'] = $this->NewLeadNoteModel->getdateThisyear();
            }
            elseif($time_range=='in the last 30 days'){
                $data['result'] = $this->NewLeadNoteModel->getLast30days();
            }

            elseif($time_range=='in the last 60 days'){
                $data['result'] = $this->NewLeadNoteModel->getLast60days();
            }

            elseif($time_range=='in the last 90 days'){
                $data['result'] = $this->NewLeadNoteModel->getLast90days();
            }

            elseif(!empty($from and $too)){
                $from1=$from;
                $too1=$too;
                $data['result'] = $this->NewLeadNoteModel->getFromToo($from1,$too1);
            }
            else{  
            $data['result'] = $this->NewLeadNoteModel->getResultAdmin($tableName,$DbKey,$idG,$order_id);
            }
            $data['searchuser'] = $this->NewLeadNoteModel->Searchuser();
         }
         else{
            if(!empty($gsearch)){
                $data['result'] = $this->NewLeadNoteModel->getSearchInput_admin_user($gsearch,$id);
            }
            elseif(!empty($date_data_sale)){
                $data['result'] = $this->NewLeadNoteModel->getDateofSale_user($date_data_sale,$id);
            }
            elseif(!empty($lead_status)){
                $data['result'] = $this->NewLeadNoteModel->getLeadStatus_user($lead_status,$id);
            }
            elseif($getdays=='days'){
                $getnumber1=$getnumber;
                $data['result'] = $this->NewLeadNoteModel->getdatedays_user($getnumber1,$id);
            }
            elseif($getdays=='Weeks'){
                $getnumber1=$getnumber;
                $data['result'] = $this->NewLeadNoteModel->getdateweek_user($getnumber1,$id);
            }
            elseif($getdays=='Months'){
                $getnumber1=$getnumber;
                $data['result'] = $this->NewLeadNoteModel->getdatemonths_user($getnumber1,$id);
            }
            elseif($time_range=='Today'){
                $data['result'] = $this->NewLeadNoteModel->getdatetoday_user($id);
            }
            elseif($time_range=='This week'){
                $data['result'] = $this->NewLeadNoteModel->getdateThisweek_user($id);
            }
            elseif($time_range=='This month'){
                $data['result'] = $this->NewLeadNoteModel->getdateThismonth_user($id);
            }
            elseif($time_range=='This year'){
                $data['result'] = $this->NewLeadNoteModel->getdateThisyear_user($id);
            }
            elseif($time_range=='in the last 30 days'){
                $data['result'] = $this->NewLeadNoteModel->getLast30days_user($id);
            }

            elseif($time_range=='in the last 60 days'){
                $data['result'] = $this->NewLeadNoteModel->getLast60days_user($id);
            }

            elseif($time_range=='in the last 90 days'){
                $data['result'] = $this->NewLeadNoteModel->getLast90days_user($id);
            }

            elseif(!empty($from and $too)){
                $from1=$from;
                $too1=$too;
                $data['result'] = $this->NewLeadNoteModel->getFromToo_user($from1,$id,$too1);
            }
            else{

                $data['result'] = $this->NewLeadNoteModel->getResultUser($id);
            }
        }
        $data['activeSender'] = active_mail(); 
            
            $this->load->view('include/header',$data);
            $this->load->view('newlead/list',$data);
            $this->load->view('include/footer',$data);
    ...
    public function save() {
		...
		$data = $this->NewLeadNoteModel->saveEmp();
        // log activity 
        $insert_id = $this->db->insert_id();
        $user_id           = $userid;
        $id           = $insert_id;
        $log_title         = "Add Notes New Lead Module Sales Crm  - " . $this->input->post('note');
        $log_create        = app_log_manage($user_id, $id, $log_title);
        // log activity
    ...
    function show(){
        $idG = $this->input->get('id');
        $data=$this->NewLeadNoteModel->employeeList($idG);
		echo json_encode($data);
    ...
    function delete(){
        $data['user_detail'] = $this->auth->user_info();
        $userid = $data['user_detail']->id; 
        $data=$this->NewLeadNoteModel->deleteEmp();
        // log activity 
        $insert_id = $this->input->post('id');
        $user_id           = $userid;
        $id           = $insert_id;
        $log_title         = "Delete Notes id New Lead Module Sales Crm - " . $id;
        $log_create        = app_log_manage($user_id, $id, $log_title);
        // log activity
		echo json_encode($data);
	}
    function updatenote(){
        $data['user_detail'] = $this->auth->user_info();
        $userid = $data['user_detail']->id; 
        $data=$this->NewLeadNoteModel->updateEmp();
        // log activity 
        $insert_id = $this->input->post('id');
        $user_id           = $userid;
        $id           = $insert_id;
        $log_title         = "Update Notes New Lead Module  Sales Crm - " . $this->input->post('note');
        $log_create        = app_log_manage($user_id, $id, $log_title);
        // log activity 
		echo json_encode($data);
	}
    ...
    {   
        ...
        $user_id           = $userid;
        $id           = $insert_id;
        $log_title         = "Delete Email New Lead Module Sales Crm - " . $ids;
        $log_create        = app_log_manage($user_id, $id, $log_title);
        // log activity 
    ...
    {   $data['user_detail'] = $this->auth->user_info();
        ...
        $user_id           = $userid;
        $id           = $insert_id;
        $log_title         = "Delete Email Drafts New Lead Module Sales Crm - " . $ids;
        $log_create        = app_log_manage($user_id, $id, $log_title);
        // log activity
    ...
    {   
        $data['user_detail'] = $this->auth->user_info();
        $ids = $this->input->post('ids');
        $delete = $this->NewLeadNoteModel->deleteLead($ids);  
        // log activity 
        $insert_id = $ids;
        $user_id           = $data['user_detail']->id;
        $id           = $insert_id;
        $log_title         = "Delete New Lead id Sales Crm - " . $ids;
        $log_create        = app_log_manage($user_id, $id, $log_title);
        // log activity 
        if($delete){
            $_SESSION['done'] =  'Lead Deleted successfully';
            redirect(base_url().'sales/list-newlead');
        }
        else{
            $_SESSION['error'] =  'Lead Data Not Added successfully';
            redirect(base_url().'sales/list-newlead');
        }
    }  
    ...
    public function emaildraft(){
        ...
        $insert = $this->NewLeadNoteModel->insertDraft($savedraft);
        // log activity 
        $insert_id = $this->db->insert_id();
        $user_id           = $data['user_detail']->id;
        $id           = $insert_id;
        $log_title         = "Add Email Draft New Lead Module Sales Crm  - " . json_encode($savedraft);
        $log_create        = app_log_manage($user_id, $id, $log_title);
        // log activity
        if($insert){
            echo 1;
   ...
   public function mail(){
    ...
    $save = $this->NewLeadNoteModel->sendEmail($insert);
    // log activity 
    $insert_id = $this->db->insert_id();
    $user_id           = $userid;
    $id           = $insert_id;
    $log_title         = "Add Email And Send Email New Lead Module Sales Crm  - " . json_encode($insert);
    $log_create        = app_log_manage($user_id, $id, $log_title);
    // log activity
...
public function fetchtemplate(){
    ...
    $data = $this->NewLeadNoteModel->fetchTemplate($id);
   echo  json_encode($data);

}
public function getLeadUnqiue($email,$mobile,$lead_id="")
{
       if($this->LeadNoteModel->getDuplicateLead($email,$mobile,$lead_id))
       {
        return true;
       }
}
