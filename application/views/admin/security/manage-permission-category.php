                                ...
                                <div class="d-flex align-items-start">
                                    <div class="tab-content w-100" id="v-pills-tabContent">
                                        <div class="tab-pane fade show active" id="v-pills-home" role="tabpanel" aria-labelledby="v-pills-home-tab" tabindex="0">
                                            <div class="row">
                                                <form action="<?php echo base_url('security/addPermissionCategory'); ?>" method="POST">
                                                    <div class="row mt-3">
                                                        <div class="form-group col-md-6">
                                                            <div class="mb-3">
                                                                <label class="form-label">Permission Category</label>
                                                                <input type="text" class="form-control" name="title" value="<?php if($edit_id) { echo $permission_cat->title; } else { echo set_value('title'); } ?>">
                                                                <?php
                                                                if($edit_id)
                                                                {
                                                                ?>
                                                        ...
                                                        foreach ($result as $row) { ?>
                                                            <tr>
                                                                ...
                                                                <?php if ($row->status == 1) {?>
                                                                    ...
                                                                    <td><label class="switch"><input type="checkbox" checked class="permission_cat" data-id="<?=$row->id?>"><span class="slider round"></span></label></td>
                                                                <?php } else {?>
                                                                    <td><label class="switch"><input type="checkbox" class="permission_cat" data-id="<?=$row->id?>"><span class="slider round"></span></label></td>

                                                                <?php } ?>
                                                                <td>
                                                                    <!-- <span><a href="javascript:void(0);"><i class="fa-solid fa-pen-to-square text-success icons-i"></i></a></span> -->
                                                                    <span><a href="<?=base_url()?>security/permission-category?edit=<?=base64_encode($row->id)?>"><i class="fa-solid fa-edit text-secondary"></i> </a></span>
                                                                    <span class="category_deleted" data-id="<?=$row->id?>"><i class="fa-regular fa-trash-can text-danger icons-i"></i></span>
                                                                </td>
                                                            </tr>
                                                    <?php $n++;
                                                        }
